...
class VibeMemoryLifecycleTest(unittest.TestCase):
    ...
    def test_hooks_repair_rejects_marker_only_launcher_before_writing(self) -> None:
        launcher = self.paths.launcher
        ...
        state = {"current_version": "1.0.0", "installed_clients": ["codex"]}

        with mock.patch(
            "vibe_memory_cli.vibe_memory_install.read_install_state", return_value=state
        ), mock.patch(
            "vibe_memory_cli.vibe_memory_install._current_version", return_value="1.0.0"
        ), mock.patch(
            "vibe_memory_cli.vibe_memory_install._managed_release_version", return_value="1.0.0"
        ), mock.patch.object(
            vibe_memory_cli, "_persisted_python_status", return_value=(sys.executable, None)
        ), mock.patch(
            "vibe_memory_cli.vibe_memory_install.read_runtime_config", return_value={}
        ), mock.patch(
            "vibe_memory_cli.vibe_memory_hooks.repair"
        ) as repair:
            code, output, stderr = self.invoke(["hooks", "repair"])
        ...
        self.assertEqual(code, 1)
        self.assertIsNone(output)
        self.assertEqual(stderr, "hooks failed: hooks repair launcher is missing or invalid\n")
        repair.assert_not_called()
    ...
    def test_hooks_repair_rolls_back_all_attempted_hooks_on_later_failure(self) -> None:
        ...
        def repair(target: pathlib.Path, *_args: object, **_kwargs: object) -> dict[str, object]:
            ...
            if target == claude:
                error = RuntimeError("second client failed")
                error._commit_snapshot = committed
                raise error
            return {"status": "current", "changed": True, "_commit_snapshot": committed}
    ...
    def test_hooks_repair_preserves_replacement_inside_failing_repair(self) -> None:
        codex = self.home / ".codex/hooks.json"
        codex.parent.mkdir(parents=True)
        codex.write_text('{"before":"codex"}\n', encoding="utf-8")
        concurrent = b'{"third_party":true}\n'
        ...
        def repair(target: pathlib.Path, *_args: object, **_kwargs: object) -> dict[str, object]:
            ...
            raise RuntimeError("repair failed after third-party replacement")
